...
| ID | 任务 | 状态 | 估时 |
|----|------|------|------|
| T10.1.1 | funds.phase 语义拆分：拆为 valuation_level（low/normal/high）+ lifecycle_stage（dca/holding），迁移现有数据 | [ ] | 0.5d |
...
| T10.1.6 | 基金资产类型字段扩展（asset_type：equity/bond/gold/qdii/index），用于例外规则匹配 | [ ] | 0.25d |
| T10.1.7 | 市场指数表 market_index_history（沪深300/中证500 等），定时拉取 | [ ] | 0.5d |
