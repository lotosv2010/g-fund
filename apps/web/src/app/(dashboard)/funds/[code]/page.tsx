...
import {
  ...
  Space, Skeleton, message, Alert, Divider, Progress, Switch, InputNumber,
} from "antd";
import {
  ArrowLeftOutlined, FundOutlined,
  ScheduleOutlined, ControlOutlined,
} from "@ant-design/icons";
import type {
  FundListItem, DcaCalculation,
...
import { fundsApi, dcaApi, rulesApi } from "@/lib/api-client";
...
export default function FundDiagnosisPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [fundData, dcaData, overridesData] = await Promise.allSettled([
      ...
      <Space orientation="vertical" size="middle" style={{ width: "100%" }}>
        <Card styles={{ body: { padding: "12px 24px" } }}>
          ...
          <Text type="secondary" style={{ fontSize: 12, display: "block", marginTop: 4 }}>
            基金画像 · 估值 / 定投 / 规则配置。持仓信号请前往「交易与持仓」页查看。
          </Text>
        </Card>

        <Card title={<><ScheduleOutlined /> 定投建议</>}>
          {!dca ? (
